...
search_modrinth() {
    ...
    local url="${MODRINTH_API}/search"
    local params=""

    # --------------------------------------------------------
    # Query is optional.
    #
    # Empty query = browse/popular projects.
    # We DO NOT send query= in that case.
    # --------------------------------------------------------
    ...
    url+="limit=${limit}&index=relevance"
...
browse_modrinth() {
    ...
    echo >&2
    echo -e "${CYAN}Browsing Modrinth${RESET}" >&2
    echo >&2

    local result

    result="$(
        search_modrinth "" "$project_type" "$limit" 2>/dev/null
    )" || {
        echo -e "${RED}✗${RESET} Could not reach Modrinth API." >&2
        return 1
    }

    if ! jq -e '.hits' >/dev/null 2>&1 <<<"$result"; then
        echo -e "${RED}✗${RESET} Modrinth returned invalid data." >&2
        return 1
    fi

    local count
    count="$(jq '.hits | length' <<<"$result")"

    if (( count == 0 )); then
        echo -e "${YELLOW}No projects found.${RESET}" >&2
        return 0
    fi

    echo -e "${BOLD}Top ${count} ${project_type}s:${RESET}" >&2
    echo >&2
    ...
    ' <<<"$result" |
    nl -w2 -s'  ' |
    while IFS=$'\t' read -r number title slug downloads description; do
        printf "  %b%s%b %b%s%b\n" \
            "${CYAN}" \
            "$number" \
            "${RESET}" \
